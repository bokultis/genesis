...
INSERT INTO upgrade_db_log (
    ...
    repos_dt,
    insert_dt) 
values (substring_index(trim(both '$' from '$HeadURL: svn+sshp://radisa.no-ip.info/repos/web/apps/cms/branches/horisen_2015/docs/db/scripts/updb0017-teaser_module.sql $'),'/',-1),
    'horisen_2015',
    trim(TRAILING '$' from substring('$Author: milan $',locate(':','$Author: milan $')+1)),
    trim(TRAILING '$' from substring('$Revision: 25141 $',locate(':','$Revision: 25141 $')+1)),
    trim(TRAILING '$' from substring('$Date: 2014-11-18 10:20:29 +0100 (уто, 18 нов 2014) $',locate(':','$Date: 2014-11-18 10:20:29 +0100 (уто, 18 нов 2014) $')+1)),
    now());
