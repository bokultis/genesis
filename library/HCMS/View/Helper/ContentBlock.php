...
class HCMS_View_Helper_ContentBlock extends Zend_View_Helper_Abstract {
    ...
    public function contentBlock($id, $lang = null, $findBy = 'url_id') {
        if(!isset($lang)){
            $lang = CURR_LANG;
        }
        $block = new Cms_Model_Page();
        if($findBy == 'url_id'){
            $result = Cms_Model_PageMapper::getInstance()->findByUrlId($id , $this->_application_id, $block, $lang, $this->_type);
        }
        else{
